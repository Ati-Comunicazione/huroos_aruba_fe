                                ...
                                for elem in contents:
                                    for inv in elem['invoices']:
                                        ...
                                        if inv['status'] not in SDI_COMPLETED:
                                            invoice_id = self.env['account.move'].search([('name', '=', inv['number'])])
                                            if invoice_id:
                                                ...
                                                if attachment:
                                                    ...
                                                    # data_notifications = {
                                                    #     'invoiceFilename': elem['filename'],
                                                    # }
                                                    # notification_response = requests.get(ws.web_server_method_address + WS_ENDPOINT_NOTIFICATION_FILENAME_CORRECT,headers=header, params=data_notifications)
                                                    # notification_response = notification_response.json()
                                                    # sleep(2)
                                                    # notification = [(5,)]
                                                    #
                                                    # for notify in notification_response['notifications']:
                                                    #     # notification_date = re.sub(r'([-+]\d{2}):(\d{2})(?:(\d{2}))?$', r'\1\2\3', notify['notificationDate'])
                                                    #     # date = datetime.datetime.strptime(notification_date, '%Y-%m-%dT%H:%M:%S.%f%z')
                                                    #     notification.append((0, 0, {
                                                    #         'sdi_state': notify['result'],
                                                    #         # 'date': date.strftime('%Y-%m-%d %H:%M:%S'),
                                                    #         'sdi_description': notify['errorDescription']
                                                    #     }))
                                                    #
